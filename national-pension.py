...
class PensionData():
    ...
    def company_info(self, company_name): # 검색한 기업을 정보 출력
        company = self.find_company(company_name)
        return self.df.loc[company.iloc[0].name]
        
    def get_data(self): # 데이터를 받음
        return self.df

@st.cache_data # 맨 처음 실행할 때 데이터를 읽어 저장, 데이터를 읽는 로딩이 반복되지 않음
def read_pensiondata():
    data = PensionData('https://www.dropbox.com/s/nxeo1tziv05ejz7/national-pension.csv?dl=1')
    return data

data = read_pensiondata()
company_name = st.text_input('회사명을 입력해 주세요', placeholder='검색할 회사명 입력') # 검색한 회사명을 company_name에 저장

if data and company_name: # 저장된 company_name으로 find_company(검색)
    output = data.find_company(company_name=company_name)
    if len(output) > 0: # 검색한 결과가 0보다 크면 = 검색이 되었으면 -> 아래 로직 실행
